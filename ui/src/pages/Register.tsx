...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setError(""); // Clear any previous error messages before starting
    setIsLoading(true); // Assuming you have a loading state to disable the button
    ...
    try {
      const response = await fetch("http://localhost:3001/api/auth/signup", {
        ...
        body: JSON.stringify({
          username, // Make sure 'username', 'email', and 'password' match your state variables
          email,
          password,
        }),
      });
      ...
      if (!response.ok) {
        // If the server returns a 400 or 500 error, throw it to the catch block
        throw new Error(data.message || "Registration failed");
      }

      // 1. Save the newly issued JWT token to client storage
      localStorage.setItem("token", data.token);

      // 2. Redirect the user to the chat screen or dashboard
      navigate("/chat");
    } catch (err: any) {
      // Catch validation or server errors and display them in your UI
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background p-4 transition-colors duration-200">
      ...
      <div className="w-full max-w-md bg-surface border border-border rounded-2xl shadow-sm p-8 space-y-6">
        {/* Branding & Headers */}
        <div className="text-center space-y-2">
          <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-primary/10 text-primary mb-2">
            {/* Simple App Identity Icon */}
            <svg
              className="w-6 h-6"
              fill="none"
              stroke="currentColor"
          ...
          <p className="text-sm text-text-secondary">
            Join Chitchat today. It's fast and completely secure.
          </p>
        </div>
        ...
        <form className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            {/* The container MUST be relative so the button absolute positioning works inside it */}
            <div className="relative flex items-center">
              ...
              {/* Input Field - Changed type to dynamically check showPassword, added pr-10 */}
              <input
              ...
              {/* Right Button (Eye Toggle) - Placed INSIDE the relative container and vertically centered */}
              <button
                type="button"
                onClick={() => setShowPassword((prev: boolean) => !prev)}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-text-muted hover:text-text transition focus:outline-none flex items-center justify-center"
          ...
          <button
            type="submit"
            className="w-full mt-2 py-3 px-4 bg-primary hover:bg-primary-dark text-white font-medium rounded-xl shadow-sm shadow-primary/10 hover:shadow-md transition-all duration-150 transform active:scale-[0.98]"
          >
            Sign Up
          </button>
        </form>
